        ...
        {proposals.length > 0 ? (
          <div className="mygigs-list">
            {proposals.map((proposal) => (
              <div
                key={proposal._id}
                className={`gig-card fade-in ${formatStatusClass(proposal.gig.status)}`}
              >
                {proposal.gig ? (
                  <>
                    ...
                    <div className="gig-actions">
                      <Link to={`/gig/${proposal.gig._id}`} className="action-btn view">View Gig</Link>
                    </div>
                  </>
                ) : (
                  <p className="no-proposals">Gig details not found. It may have been deleted.</p>
                )}
              </div>
            ))}
          </div>
